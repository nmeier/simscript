...
#  | GEAR HANDLE POSITION | True if gear handle is applied | Bool | Y | All aircraft |
# see http://msdn.microsoft.com/en-us/library/cc526980.aspx#AircraftMiscellaneousSystemsIDs
...
for i in range(0,abs(delta)):
    fsx.set("COM_RADIO_WHOLE_INC" if delta>0 else "COM_RADIO_WHOLE_DEC")

encoder2 = phidgets.get(82081)
delta = phidgets.encoder2delta(encoder2)
for i in range(0,abs(delta)):
    fsx.set("COM_RADIO_FRACT_INC" if delta>0 else "COM_RADIO_FRACT_DEC")

if state.toggle("swap_radio", encoder1.getInputState(0)):
    fsx.set("COM_STBY_RADIO_SWAP")
